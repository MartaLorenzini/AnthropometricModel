...
%% HAND (box)
% Geometric dimensions
handAlfa = foreArmDiameter; %arbitrarily defined as the width of the forearm
handBeta = 0.108*H;
handGamma = foreArmRadius; %arbitrarily defined as the half depth of the forearm
handHalfBeta = 0.5*handBeta;
% Mass and inertia
handMass = 0.006*M;
handIxx = (handMass/12)*(handAlfa^2 + handBeta^2);
handIyy = (handMass/12)*(handBeta^2 + handGamma^2);
handIzz = (handMass/12)*(handGamma^2 + handAlfa^2);
...
% HAND
subjectParams(subjectID).handAlfa = handAlfa;
...
subjectParams(subjectID).ghostDim = ghostDim;
subjectParams(subjectID).ghostMass = ghostMass;
subjectParams(subjectID).ghostIn = ghostIn; 
